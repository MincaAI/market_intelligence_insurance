# Script Playwright pour capturer la partie basse de la page à partir de 'Avis de la clientèle'
...
def get_screenshot_path(product):
    ...
    screenshots_dir = os.path.join(base_dir, 'data', 'reviews_screenshots', 'generali')
...
async def capture_bottom_from_avis_client(product='travel', hauteur_voulue=800):
...
if __name__ == "__main__":
    import sys
    # Usage: python generali_reviews.py [car|travel] [hauteur]
    product = 'travel'
    hauteur = 800
    ...
    if len(sys.argv) > 2:
        try:
            ...
            pass
    asyncio.run(capture_bottom_from_avis_client(product, hauteur)) 