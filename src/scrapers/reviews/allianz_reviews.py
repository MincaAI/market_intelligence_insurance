# Script Playwright pour capturer la partie basse de la page à partir de 'Ce que disent nos clientes et clients' (Allianz)
...
def get_screenshot_path(product):
    ...
    screenshots_dir = os.path.join(base_dir, 'data', 'reviews_screenshots', 'allianz')
...
async def capture_bottom_from_reviews(product='travel', hauteur_voulue=800):
...
if __name__ == "__main__":
    import sys
    # Usage: python allianz_reviews.py [car|travel] [hauteur]
    product = 'travel'
    hauteur = 800
    ...
    if len(sys.argv) > 2:
        try:
            ...
            pass
    asyncio.run(capture_bottom_from_reviews(product, hauteur)) 